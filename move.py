...
def move(pos_list_initial, pos_list_end, angle):
    ...
    ratio = 0.1
    length = 80   # 80
    offset = np.array([0, -70, 0])
    # offset1 = np.array([-5, -20, 0])
    # offset2 = np.array([-5, 20, 0])
    perp = Rz(pi/2).dot(pb) / np.linalg.norm(pb)
    pb1 = pb0 + pb/4*0 + np.array([0, 0, d4])
    pb2 = pb0 + pb * ratio + length * perp + np.array([0, 0, d4]) + offset
    pb3 = pb0 + pb * ratio*3 - length*perp + np.array([0, 0, d4])
    pb4 = pb0 + pb * (1 - ratio*3) + length*perp + np.array([0, 0, d4])
    pb5 = pb0 + pb * (1 - ratio) - length*perp + np.array([0, 0, d4]) - offset + np.array([0,10,0])
